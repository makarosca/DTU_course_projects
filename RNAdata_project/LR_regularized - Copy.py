...
# IT STANARDIZES THEM AGAIN LOL
    ...
    y_est = X_test @ w_noreg[:,-1]; y_true = y_test
    #y_est = X[100:400,:] @ w_rlr[:,-1]; y_true = y[100:400]
    axis_range = [np.min([y_est, y_true])-1,np.max([y_est, y_true])+1]
    plt.plot(axis_range,axis_range,'k--')
    plt.plot(y_true, y_est,'ob',alpha=.25)
    y_est = X @ w_rlr[:,-1]; y_true = y
    plt.plot(y_true, y_est,'or',alpha=.25)
