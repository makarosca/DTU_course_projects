...
def ann_model_selection(X, y, K, h_values, k = 0):
...
# Values of lambda and h (number of hidden neurons)
...
for train_index, test_index in CV.split(X,y):
    ...
    E_test_ANN[k] = ann_model_test(np.copy(X[train_index]), np.copy(y[train_index]), np.copy(X[test_index]), np.copy(y[test_index]), h_opt_temp)
